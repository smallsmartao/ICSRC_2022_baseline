...
data=/your/data/aishell/
dev_data=/your/data/dev_release/
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  mkdir data/dev_p5                                                                                                                                                                                                                                                                                                     
  awk -v tmp=$dev_data '{print $1 " "tmp"/"$2}' $dev_data/wav.scp > data/dev_p5/wav.scp
  cp  $dev_data/text  data/dev_p5/text
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  tools/make_raw_list.py data/dev_p5/wav.scp data/dev_p5/text \
    data/dev_p5/data.list
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  for mode in ${decode_modes}; do
  {
    ...
    python wenet/bin/recognize.py --gpu 0 \
      ...
      --data_type $data_type \
      --test_data data/dev_p5/data.list \
    ...
    python tools/compute-wer.py --char=1 --v=1 \
      data/dev_final/text $test_dir/text > $test_dir/wer
  } &
  done
  wait
fi
